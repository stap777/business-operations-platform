...
interface OrdersDashboardSectionProps {
  title?: string;
  showCreateButton?: boolean;
}

export const OrdersDashboardSection: React.FC<OrdersDashboardSectionProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-[#0F0F0F] border border-[#ECECEC] dark:border-[#232323] rounded-xl p-5 shadow-sm space-y-4">
      {/* Section Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#ECECEC] dark:border-[#232323] pb-3">
        <div className="flex items-center gap-2">
          <ShoppingBag className="w-4 h-4 text-[#71717A] dark:text-[#A1A1AA]" />
        ...
        {showCreateButton && (
          <Button
            ...
            className="bg-[#111111] text-white dark:bg-[#FAFAFA] dark:text-[#111111] hover:opacity-90 text-xs font-semibold gap-1.5 self-start sm:self-auto rounded-lg shadow-sm"
          >
      ...
      {isError && (
        <div className="p-3 rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800/40 text-xs text-red-600 dark:text-red-400 flex items-center justify-between">
          <span>Unable to load orders data from server.</span>
