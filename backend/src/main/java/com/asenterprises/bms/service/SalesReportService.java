...
public class SalesReportService {
    ...
    private List<SalesPeriodItemResponse> buildPeriodBreakdown(LocalDate start, LocalDate end, String granularity) {
        ...
        while (!current.isAfter(end)) {
            ...
            LocalDateTime pEnd = current.atTime(LocalTime.MAX);
            ...
            BigDecimal pAvg = pOrders > 0
                    ? pRevenue.divide(BigDecimal.valueOf(pOrders), 2, RoundingMode.HALF_UP)
                    : BigDecimal.ZERO;

            items.add(SalesPeriodItemResponse.builder()
                    .periodLabel(current.toString())
                    .totalOrders(pOrders)
                    .revenue(pRevenue)
